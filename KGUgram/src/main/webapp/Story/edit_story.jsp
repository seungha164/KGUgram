...
<%
	...
	Story mStory = storyDao.getStory(s_id);
	...
	<header>
		<div class="navbar" style="background-color:#fff;border-bottom:1px solid #f0f0f0;height:50px">
			<div class="container">
				...
				<strong><fmt:message key="story_edit"/></strong>
				<div></div>
			</div>		
		</div>
	</header>
...
<div class="container">
<p class="text-end" style="color:#000;"><a href="?s_id=<%=s_id %>&language=ko" style="text-decoration:none">Korean </a>|<a href="?s_id=<%=s_id %>&language=en" style="text-decoration:none"> English</a>
    
	<form method="post" name="fileForm" enctype="multipart/form-data" action="edit_story_process.jsp" onsubmit="return upload_validation();">
		<!--  1. 익명 여부  -->
	<div class="float-end">
		<div class="mb-3">
		<%
			if(mStory.isIs_anonymous()){
				%>
				<div class="form-check form-check-inline">
  					<input class="form-check-input" type="radio" name="is_anonymous" id="inlineRadio1" value="False">
  					<label class="form-check-label" for="inlineRadio1"><fmt:message key="nickname"/></label>
				</div>
				<div class="form-check form-check-inline">
  					<input class="form-check-input" type="radio" name="is_anonymous" id="inlineRadio2" value="True" checked>
	...
	<div class="mb-3">
  		...
  		<textarea class="form-control" id="exampleFormControlTextarea1" rows="3" name="content"><%= mStory.getContent() %></textarea>
	</div>
	<!--  3. 위치 추가  -->
	<div class="mb-3">
	...
	<div class="mb-3">
  		...
  		<input class="form-control" type="file" id="formFile" name="filename" disabled>
	</div>
	<!-- HIDDEN - S_id -->
	...
	<script type="text/javascript">
		document.getElementById('latitude_marker').value = <%= mStory.getLatitude() %>;
		document.getElementById('longitude_marker').value = <%= mStory.getLongitude() %>;
    	
		var container = document.getElementById('map'); //지도를 담을 영역의 DOM 레퍼런스
		var options = { //지도를 생성할 때 필요한 기본 옵션
			center: new kakao.maps.LatLng(<%=mStory.getLatitude() %>, <%=mStory.getLongitude() %>), //지도의 중심좌표.
